...
folder_path_styles = 'C:\\Users\\Diop Serigne Rawane\\.cache\\kagglehub\\datasets\\paramaggarwal\\fashion-product-images-dataset\\versions\\1\\fashion-dataset\\styles/'
folder_path_images = 'C:\\Users\\Diop Serigne Rawane\\.cache\\kagglehub\\datasets\\paramaggarwal\\fashion-product-images-dataset\\versions\\1\\fashion-dataset\\images/'
folder_path_all = 'all/'
...
def add_image_to_sub_category(filename, name_folder_etiquette):
    """
    Fonction pour copier l'image dans le dossier approprié basé sur la catégorie.
    """
    # Copie de l'image
...
def sub_traitement(sub_category, filename, bool_make_fold):
    """
    Fonction pour traiter chaque sous-catégorie et placer les images dans les bons dossiers.
    """
    if sub_category == 'Shoes':
        if bool_make_fold: os.makedirs(folder_path_all + 'shoes', exist_ok=True)
        add_image_to_sub_category(filename, 'shoes')
    elif sub_category == 'Topwear':
        if bool_make_fold: os.makedirs(folder_path_all + 'topwear', exist_ok=True)
        add_image_to_sub_category(filename, 'topwear')
    elif sub_category == 'Bottomwear':
        if bool_make_fold: os.makedirs(folder_path_all + 'bottomwear', exist_ok=True)
        add_image_to_sub_category(filename, 'bottomwear')
    else:
        print(f"Sous-catégorie non prise en charge : {sub_category}")
    ...
    try:
        ...
        with open(json_file, 'r', encoding='utf-8') as f:
            ...
            if sub_category in json_categorie_vetement:
                ...
                sub_traitement(sub_category, filename, False)
            else:
                # Si la sous-catégorie nous intéresse, on la crée dans le dictionnaire
                json_categorie_vetement[sub_category] = 1
                sub_traitement(sub_category, filename, True)
